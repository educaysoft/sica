...
<head>
	...
	$data=$data.'
		...
		<img src="https://repositorioutlvte.org/Repositorio/qr/produccioncientifica.png" height="150px">
		</div>
      		<div >
        	<h1 class="fw-light">UNIVERSIDAD TÉCNICA LUIS VARGAS TORRES DE ESMERALDAS</h1>  
        	<p class="lead text-muted" style="text-transform:uppercase;text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.5); " >Carrera en Tecnología de la Información </p>
        	<p class="lead text-muted"  style="text-transform:uppercase;text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.5); ">Producción Científica</p>
      		</div>
    		</div>
  		</section>
...
foreach($publicaciondocente as $rowj){
			...
			$data=$data.' <div class="card-body" style="background-color:#D3D3D3;padding: 20px;border-radius: 5px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2) "  ><b>'.$rowj[$row->iddocente]['tipo'] .': </b><span style="color:red">'.$rowj[$row->iddocente]['titulo'].'('.$rowj[$row->iddocente]['fechapublicacion'].'),</span> - link:<a href="'.$rowj[$row->iddocente]['url'].'"> <i class="fas fa-book" style="font-size:24px" ></i> </a></div><br>';
			}	
	//		echo $rowj; echo '<br>';
		}
...
';
			...
			$file='application/views/web/produccioncientifica.php';
