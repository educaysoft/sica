...
foreach($aulas as $row){
	...
	$data=$data.'
		...
		<img src="https://repositorioutlvte.org/Repositorio/qr/aulas.png" height="150px">
		</div>
      		<div >
...
$data=$data.' <input type="file" id="fileInput'.trim($row->idaula).'" accept="image/*">
  <button onclick="uploadImage(\'aula'.trim($row->idaula).'.jpg\',\''.trim($row->idaula).'\')">Subir Imagen</button>
  <p id="status'.trim($row->idaula).'"></p> </div>';
...
';
	 	...
	 	$data=$data.$data1;
			...
			$file='application/views/web/aulas.php';
