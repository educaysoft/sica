...
//		document.getElementById('idpersona').setAttribute('size',"'"+l+"'");
		for(i=0; i<data.length; i++){
			if(data[i].idtipoasistencia==" "){
				html += '<option style="color:red;" value='+data[i].idpersona+'><i class="fas fa-female"></i>'+data[i].idpersona+' - '+data[i].nombres+' - '+data[i].idtipoasistencia+'</option>';
			}else{
				html += '<option style="color:green;"  value='+data[i].idpersona+'><i class="fas fa-female"></i> - '+data[i].idpersona+'- '+data[i].nombres+' - '+data[i].idtipoasistencia+'</option>';
			}
		}
		$('#idpersona').html(html);
