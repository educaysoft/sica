...
// ==============================================
	...
	function evento_data()
	{
			...
			foreach($data0->result() as $r){
				$data[]=array($r->idevento,$r->titulo,$r->fechainicia,$r->estado,$r->lainstitucion,
					$r->href='<a href="javascript:void(0);" class="btn btn-info btn-sm item_ver"  data-retorno="'.site_url('evento/detalle').'"    data-idevento="'.$r->idevento.'">Edit</a>',$r->href='<a href="javascript:void(0);" class="btn btn-info btn-sm item_ver2"  data-retorno2="'.site_url('evento/detalle').'"    data-idevento2="'.$r->idevento.'">Ver</a>');
			}	
	...
	function evento_fechasAsisPartPago()
	{
			...
			$data=array();
			foreach($data0->result() as $r){
				$data[]=array($r->idevento,$r->fecha,$r->temacorto,$r->asistencia,$r->longitud,$r->latitud,$r->participacion,$r->pagos,
				$r->href='<a href="javascript:void(0);" class="btn btn-info btn-sm item_ver"  data-retorno="'.site_url('evento/actual').'"    data-idevento="'.$r->idevento.'"  data-fecha="'.$r->fecha.'"  data-participacion="'.$r->participacion.'"   >Part</a>');
			}	
			$output=array( "draw"=>$draw,
				"recordsTotal"=> $data0->num_rows(),
				"recordsFiltered"=> $data0->num_rows(),
