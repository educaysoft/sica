...
// ==============================================
	...
	function evento_fechas()
	{
			...
			$data=array();
			foreach($data0->result() as $r){
				$data[]=array($r->idsesionevento,$r->fecha,$r->horainicio,$r->horafin,$r->numerosesion,$r->tema, 
	...
	function evento_participantes()
	{
			...
			foreach($data0->result() as $r){
				...
				$r->href='<a href="javascript:void(0);" class="btn btn-info btn-sm item_ver"  data-retorno="'.site_url('participante/actual').'"    data-idparticipante="'.$r->idparticipante.'">Ver</a><a href="javascript:void(0);" class="btn btn-info btn-sm item_ver"  data-retorno="'.site_url('persona/actual').'"    data-idparticipante="'.$r->idpersona.'">per</a>');
			}	
			$output=array( "draw"=>$draw,
				"recordsTotal"=> $data0->num_rows(),
				"recordsFiltered"=> $data0->num_rows(),
