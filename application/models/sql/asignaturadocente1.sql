...
create view asignaturadocente1 as select asdo.idasignaturadocente ,dist.iddistributivo, asdo.iddistributivodocente,pers.idpersona, dido.iddocente,concat(COALESCE(peac.nombrecorto,'')," - ", COALESCE(pers.apellidos,''),"  ",COALESCE(pers.nombres,'')) as eldistributivodocente, dist.idperiodoacademico,peac.nombrecorto as elperiodoacademico,asig.idasignatura,concat(COALESCE(arco.nombre,'')," - ",COALESCE(asig.nombre,'')) as laasignatura,para.idparalelo, para.nombre as paralelo,niac.numero as numeronivel,niac.idnivelacademico, niac.nombre as nivel,(select round(sum(jodo.duracionminutos)/60,1) from jornadadocente jodo where jodo.idasignaturadocente=asdo.idasignaturadocente) as horas,(select hoas.cantidad from horasasignatura hoas where hoas.idasignatura=asig.idasignatura and hoas.idtipohorasasignatura=1) as hpdo, (select hoas.cantidad from horasasignatura hoas where hoas.idasignatura=asig.idasignatura and hoas.idtipohorasasignatura=2) as hpra,esad.nombre as estado,esad.idestadoasignaturadocente  from asignaturadocente asdo,distributivo dist, distributivodocente0 dido, periodoacademico peac, docente doce, persona pers,asignatura asig,paralelo para,nivelacademico niac,areaconocimiento arco,estadoasignaturadocente esad  
